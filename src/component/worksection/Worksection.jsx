"use client";
import { Appcontext } from "@/context/Appcontext";
import React, { useContext, useEffect, useState } from "react";

const Worksection = () => {
  const {
    getWorkwithus,
    createWorkwithus,
    updateWorkwithus,
    deleteWorkwithus,
  } = useContext(Appcontext);

  const emptyForm = {
    toptext: "",
    heading: "",
    paragraph: "",
    check1: "",
    check2: "",
    check3: "",
    check4: "",
    check5: "",
    check6: "",
    mainimg: "",
  };

  const [formData, setFormData] = useState({ ...emptyForm });
  const [IsEditMode, setIsEditMode] = useState(false);
  const [WorkwithusId, setWorkwithusId] = useState(null);

    const handleFileUpload = async (e, fieldName) => {
    const file = e.target.files[0];
    if (!file) return;

    const CLOUD_NAME = "dhtpqla2b";
    const UPLOAD_PRESET = "unsigned_preset";

    const uploadForm = new FormData();
    uploadForm.append("file", file);
    uploadForm.append("upload_preset", UPLOAD_PRESET);

    try {
      const res = await fetch(
        `https://api.cloudinary.com/v1_1/${CLOUD_NAME}/image/upload`,
        {
          method: "POST",
          body: uploadForm,
        }
      );

      const data = await res.json();
      console.log("Uploaded URL:", data.secure_url);

      setFormData((prev) => ({
        ...prev,
        [fieldName]: data.secure_url,
      }));
    } catch (err) {
      console.error("Upload error:", err);
    }
  };

  useEffect(() => {
    const loadWorkwithusId = async () => {
      try {
        const res = await getWorkwithus();
        const WorkwithusData = Array.isArray(res) ? res[0] : res;

        if (WorkwithusData) {
          setWorkwithusId(WorkwithusData._id);
          setIsEditMode(true);
        }
      } catch (err) {
        console.error("Failed to fetch WorkwithusId:", err);
      }
    };

    loadWorkwithusId();
  }, []);

  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setFormData((prev) => ({ ...prev, [name]: value }));
  };

  const loadForEdit = async () => {
    if (!WorkwithusId) return;

    try {
      const allWorkwithus = await getWorkwithus();
      const workwithus = allWorkwithus.find((w) => w._id === WorkwithusId);

      if (workwithus) {
        setFormData({
          toptext: workwithus.toptext || "",
          heading: workwithus.heading || "",
          paragraph: workwithus.paragraph || "",
          check1: workwithus.check1 || "",
          check2: workwithus.check2 || "",
          check3: workwithus.check3 || "",
          check4: workwithus.check4 || "",
          check5: workwithus.check5 || "",
          check6: workwithus.check6 || "",
          mainimg: workwithus.mainimg || "",
        });
      }
    } catch (err) {
      console.error("Failed to load work data for edit:", err);
    }
  };

  const submitForm = async (e) => {
    e.preventDefault();
    try {
      let res;

      if (IsEditMode && WorkwithusId) {
        res = await updateWorkwithus(WorkwithusId, formData);
        if (res) {
          alert("workwithus section updated successfully!");

          setFormData({ ...emptyForm });
        }
      } else {
        res = await createWorkwithus(formData);
        if (res) {
          alert(
            "Work section created successfully! You can now update or delete it."
          );

          setFormData({ ...emptyForm });
          setWorkwithusId(res._id);
          setIsEditMode(true);
        }
      }
    } catch (err) {
      console.error("Error submitting Work section:", err);
      alert("Something went wrong. Check the console for details.");
    }
  };

  const handleDelete = async () => {
    if (!WorkwithusId) return alert("Nothing to delete");
    if (!confirm("Are you sure you want to delete this Workwithus section?"))
      return;

    try {
      const res = await deleteWorkwithus(WorkwithusId);
      if (res?._id) {
        setFormData({ ...emptyForm });
        setWorkwithusId(null);
        setIsEditMode(false);
        alert("Work section deleted successfully!");
      }
    } catch (err) {
      console.error("Error deleting Work section:", err);
      alert("Failed to delete Work section");
    }
  };

  return (
    <div className="w-full min-h-screen bg-blue-950 overflow-auto py-10">
      <div className="w-full max-w-full mx-auto p-6">
        <form
          onSubmit={submitForm}
          className="bg-white shadow-md rounded-lg p-8 space-y-6"
        >
          <h1 className="text-3xl font-bold text-center text-gray-900">
            {IsEditMode ? "Edit Workwithus" : "Create Workwithus"}
          </h1>

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">
              Top Text
            </label>
            <input
              type="text"
              name="toptext"
              value={formData.toptext}
              onChange={handleInputChange}
              placeholder="Enter top text"
              className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none"
            />
          </div>

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">
              Main Heading
            </label>
            <input
              type="text"
              name="heading"
              value={formData.heading}
              onChange={handleInputChange}
              placeholder="Enter main heading"
              className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none"
            />
          </div>

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">
              Paragraph
            </label>
            <textarea
              name="paragraph"
              value={formData.paragraph}
              onChange={handleInputChange}
              placeholder="Enter paragraph text"
              rows="4"
              className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none"
            />
          </div>

          <div>
            <h2 className="text-lg font-semibold text-gray-800 border-b pb-2 mb-3">
              Check Points
            </h2>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Check Point 1
                </label>
                <input
                  type="text"
                  name="check1"
                  value={formData.check1}
                  onChange={handleInputChange}
                  placeholder="Enter checkpoint 1 text"
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Check Point 2
                </label>
                <input
                  type="text"
                  name="check2"
                  value={formData.check2}
                  onChange={handleInputChange}
                  placeholder="Enter checkpoint 2 text"
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Check Point 3
                </label>
                <input
                  type="text"
                  name="check3"
                  value={formData.check3}
                  onChange={handleInputChange}
                  placeholder="Enter checkpoint 3 text"
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Check Point 4
                </label>
                <input
                  type="text"
                  name="check4"
                  value={formData.check4}
                  onChange={handleInputChange}
                  placeholder="Enter checkpoint 4 text"
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Check Point 5
                </label>
                <input
                  type="text"
                  name="check5"
                  value={formData.check5}
                  onChange={handleInputChange}
                  placeholder="Enter checkpoint 5 text"
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Check Point 6
                </label>
                <input
                  type="text"
                  name="check6"
                  value={formData.check6}
                  onChange={handleInputChange}
                  placeholder="Enter checkpoint 6 text"
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none"
                />
              </div>
            </div>
          </div>

          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Main Image
            </label>

            <div className="flex items-center gap-2">
              <label className="cursor-pointer bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600">
                Upload File
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => handleFileUpload(e, "mainimg")}
                  className="hidden"
                />
              </label>

              <input
                type="text"
                name="mainimg"
                value={formData.mainimg || ""}
                onChange={handleInputChange}
                placeholder="https://example.com/main-image.jpg"
                className="flex-1 p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none"
              />
            </div>

            {formData.mainimg && (
              <img
                src={formData.mainimg}
                alt="Preview"
                className="w-24 h-24 mt-2 object-cover border rounded"
              />
            )}
          </div>

          <div className="flex gap-4">
            <button
              type="submit"
              className="w-[30%] bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition-colors duration-200"
            >
              {WorkwithusId ? "Update Work section" : "Save Work Section"}
            </button>

            {WorkwithusId && (
              <>
                <button
                  type="button"
                  onClick={loadForEdit}
                  className="w-[20%] bg-yellow-600 hover:bg-yellow-700 text-white py-2 rounded-lg font-semibold transition"
                >
                  Load for Edit
                </button>

                <button
                  type="button"
                  onClick={handleDelete}
                  className="w-[20%] bg-red-600 hover:bg-red-700 text-white py-2 rounded-lg font-semibold transition"
                >
                  Delete Work section
                </button>
              </>
            )}
          </div>
        </form>
      </div>
    </div>
  );
};

export default Worksection;
